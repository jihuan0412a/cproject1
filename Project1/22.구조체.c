...
//데이터의 구조와 작없을 진행할 떄 사용하는 데이터입니다.
...
//구조체 선언은 변수 선언은 아니기 떄문에, 사용하기 위해서 따로 변수 선언과 초기화를 진행해야 합니다.
...
int main()
{
	...
	struct student s1 = { 20425, "Keite", 'A' };
	...
	printf("이름 :");
	scanf("%s", &s3.name);
	getchar(); //문자열 다음에 문자를 받고있으니 getchar()를 통해 버퍼 비우기
	printf("그룹 :");
	scanf("%c", &s3.group);

	printf("학번 : %d이름 : %s 그룹 : %c\n", s1.number, s1.name, s1.group);
